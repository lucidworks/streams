...
@REM of Twigkit and its suppliers, if any.
@REM The intellectual and technical concepts contained herein are PROPRIETARY
@REM to Twigkit and its suppliers and may be covered by U.S. and Foreign Patents,
...
@REM Copyright (c) 2009-2016 Twigkit.  All rights reserved.
...
@REM	echo ""
@REM    echo "Could not find project pom.xml file, are you sure the current directory is a Twigkit project?"
@REM    echo ""
@REM    echo "Note - you can use ./twigkit_install to add these scripts to an existing Twigkit project. Run:"
@REM    echo "./twigkit_install /path-to-your-project/"
@REM    echo ""
@REM    exit 1
@REM fi
...
IF "%SCRIPT_CMD%" == "start" (
  @REM see if Twigkit is already running
...
:print_usage
  ...
  IF [%1]==[] (
    echo.
    echo Usage: twigkit COMMAND OPTIONS
    echo        where COMMAND is one of: start, stop, package, dist
    echo.
    echo   Allows you to build and run Twigkit applications easily from the command line.
    echo.
    echo   Standalone server example [start Twigkit running in the background on port 8080]:
    echo.
    echo     ./twigkit start -p 8080
    echo.
    echo.
    echo Pass -help after any COMMAND to see command-specific usage information,
    echo   such as:    ./twigkit start -help or ./twigkit stop -help
    echo.
	EXIT /B 0
  )
  IF %CMD%==start (
    echo.
    echo Usage: twigkit %CMD% [-f] [-p port] [-m memory] [-t timeout] [-V]
    echo.
    echo   -f            Start Twigkit in foreground; default starts Twigkit in the background
    echo                   and sends stdout / stderr to twigkit-PORT-console.log
    echo.
    echo   -p [port]     Specify the port to start the Twigkit web server on; default is 8080.
    echo.
  ...
  IF %CMD%==stop (
    echo.
    echo Usage: twigkit %CMD% [-p port] [-V]
    echo.
    echo   -p [port]     Specify the port the Twigkit HTTP listener is bound to
    echo.
    echo.
  )
  IF %CMD%==package (
    echo.
    echo Usage: twigkit %CMD%
    echo.
    echo   NOTE: This command will build a new WAR file with Twigkit and place it in ./dist
    echo.
  )
  IF %CMD%==dist (
    echo.
...
:find_and_stop_twigkit
  ...
  @REM    echo No process found for Twigkit node running on port %TWIGKIT_PORT%
  @REM    EXIT /B 1
  @REM )
EXIT /B 0

@REM tries to gracefully stop Twigkit using the Jetty
...
:stop_twigkit
  ...
  IF NOT [%3] == [] (
    echo Sending stop command to Twigkit running on port %TWIGKIT_PORT% ...
    CALL %MVNCMD% jetty:stop -Dtwigkit.http.stop.port=%STOP_PORT%
	echo Twigkit stop command executed succesfully. If Twigkit still appears to be running it may be necessary to manually kill the Java process.
    @REM (sleep 5) &
    @REM spinner $!
    @REM rm -f "%TWIGKIT_PID_DIR%/twigkit-%TWIGKIT_PORT%.pid"
  ) ELSE (
    echo No Twigkit nodes found to stop.
    EXIT /B 0
  )

  @REM CHECK_PID=`ps auxww | awk '{print $2}' | grep -w $TWIGKIT_PID | sort -r | tr -d ' '`
  @REM if [ "$CHECK_PID" != "" ]; then
  @REM   echo "Twigkit process $TWIGKIT_PID is still running; forcefully killing it now."
  @REM   kill -9 $TWIGKIT_PID
  @REM   echo "Killed process $TWIGKIT_PID"
  @REM   rm -f "$TWIGKIT_PID_DIR/twigkit-$TWIGKIT_PORT.pid"
  @REM   sleep 1
  @REM else
  @REM   echo "Twigkit process stopped gracefully"
  @REM fi

  @REM CHECK_PID=`ps auxww | awk '{print $2}' | grep -w $TWIGKIT_PID | sort -r | tr -d ' '`
  @REM if [ "$CHECK_PID" != "" ]; then
  @REM   echo "ERROR: Failed to kill previous Twigkit Java process $TWIGKIT_PID ... script fails."
  @REM   exit 1
  @REM fi
EXIT /B 0

@REM build the deployable Twigkit artifact
:build_twigkit
  CALL "%MVNCMD%" clean package --settings "%SETTINGS%"
EXIT /B 0

@REM build the deployable Twigkit artifact as a standalone service
:build_twigkit_standalone
  CALL "%MVNCMD%" clean package -Pstandalone --settings "%SETTINGS%"
EXIT /B 0

@REM Launches Twigkit in foreground/background depending on parameters
:launch_twigkit
  ...
  IF %FG%==1 (
    echo.
    echo Starting Twigkit on port %TWIGKIT_PORT%
	@echo on
    CALL %MVNCMD% jetty:run --settings %SETTINGS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false -Dtwigkit.conf=file://src/main/resources/conf
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows
  ) ELSE (
    @REM run in the background
    start /b cmd /c CALL %MVNCMD% jetty:run --settings %SETTINGS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false -Dtwigkit.conf=file://src/main/resources/conf %SOLR_OPTS%>"%TWIGKIT_LOGS_DIR%/twigkit-%TWIGKIT_PORT%-console.log" 2>&1
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows

	@REM for /F "TOKENS=1,2,*" %%a in ('tasklist /FI "IMAGENAME eq java.exe"') do set TWIGKIT_PID=%%b
	echo.
	echo Please wait while Twigkit loads...
	ping 192.0.2.2 -n 1 -w 45000 > nul
	echo Started Twigkit server on port %TWIGKIT_PORT%. Have fun!
