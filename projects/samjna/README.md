...
This application adds image-based perception capabilities to Lucidwork's [Fusion 4.x](https://lucidworks.com/products/fusion-server/) using a general AI network. As Fusion crawls, images are sent to the network which then extracts tags related to the image. These tags are then inserted by Fusion into a Solr-based search index for searching by the user.
